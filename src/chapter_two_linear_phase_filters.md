...
As a consequence, their equation does not have any \\(y\\), i.e., feedback term, and therefore the impulse response is finite (hence the name finite impulse response).
...
Note that the convolution theorem holds even when input signal is complex-valued.
